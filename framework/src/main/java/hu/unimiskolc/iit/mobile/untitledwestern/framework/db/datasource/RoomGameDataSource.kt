...
class RoomGameDataSource(private val dao: GameDao, private val mapper: GameMapper) :
    GameDataSource {
    ...
    override suspend fun fetchById(id: Int): Game? =
        dao.fetchById(id)?.let { mapper.mapFromEntity(it) }

    override suspend fun fetchAll(): List<Game> = dao.fetchAll().map { mapper.mapFromEntity(it) }

    override suspend fun fetchOrderByScore(limit: Int): List<Game> =
        dao.fetchOrderByScore(limit).map { mapper.mapFromEntity(it) }
}